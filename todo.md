...
#### 任务清单：
1. **统一Agent架构**
   - [ ] 创建`app/agents/base_agent.py`，统一工具调用和RAG流程
   - [ ] 实现意图识别：判断用户问题是订单查询还是产品推荐
   - [ ] 实现多轮对话状态管理（可选）

2. **用户认证与上下文**
   - [ ] 实现用户登录/注册接口（JWT认证）
   - [ ] 在对话API中获取当前用户身份
   - [ ] Agent根据用户身份查询个人订单/历史
...
#### 任务清单：
1. **单元测试**
   - [ ] 编写`tests/test_api.py`测试API端点
   - [ ] 编写`tests/test_agents.py`测试Agent逻辑
   - [ ] 使用pytest运行测试
